...
from .carla_data_utils import get_carla_image_info
...
def create_carla_info_file(data_path,
                           pkl_prefix='carla',
                           with_plane=False,
                           save_path=None,
                           relative_path=True):
    """Create info file of CARLA dataset.
    ...
    carla_infos_train = get_carla_image_info(
        data_path,
        training=True,
        velodyne=True,
        calib=True,
    ...
    _calculate_num_points_in_gt(data_path, carla_infos_train, relative_path)
    filename = save_path / f'{pkl_prefix}_infos_train.pkl'
    print(f'Carla info train file is saved to {filename}')
    mmcv.dump(carla_infos_train, filename)
    carla_infos_val = get_carla_image_info(
        data_path,
        training=False,
        velodyne=True,
        calib=True,
        with_plane=with_plane,
        relative_path=relative_path)
    _calculate_num_points_in_gt(data_path, carla_infos_val, relative_path)
    filename = save_path / f'{pkl_prefix}_infos_val.pkl'
    print(f'Carla info val file is saved to {filename}')
    mmcv.dump(carla_infos_val, filename)
    filename = save_path / f'{pkl_prefix}_infos_trainval.pkl'
    print(f'Carla info trainval file is saved to {filename}')
    mmcv.dump(carla_infos_train + carla_infos_val, filename)

    # carla_infos_test = get_carla_image_info(
    #     data_path,
    #     training=False,
    #     label_info=False,
    #     velodyne=True,
    ...
    # print(f'Carla info test file is saved to {filename}')
    # mmcv.dump(carla_infos_test, filename)
