...
def get_carla_info_path(idx,
    ...
    prefix = Path(prefix)
    if training:
        file_path = Path('train') / info_type / img_idx_str
    else:
        file_path = Path('val') / info_type / img_idx_str
...
def get_image_path(idx,
                   ...
                   file_tail='.png',
                   use_prefix_id=False):
    return get_carla_info_path(idx, prefix, info_type, file_tail, training,
...
def get_label_path(idx,
                   ...
                   relative_path=True,
                   exist_check=True,
                   info_type='kitti_label',
                   use_prefix_id=False):
    return get_carla_info_path(idx, prefix, info_type, '.txt', training,
                               relative_path, exist_check, use_prefix_id)
...
def get_velodyne_path(idx,
                      ...
                      exist_check=True,
                      use_prefix_id=False):
    return get_carla_info_path(idx, prefix, 'velodyne', '.bin', training,
                               relative_path, exist_check, use_prefix_id)
...
def get_calib_path(idx,
                   ...
                   exist_check=True,
                   use_prefix_id=False):
    return get_carla_info_path(idx, prefix, 'calib', '.txt', training,
                               relative_path, exist_check, use_prefix_id)
...
def get_pose_path(idx,
                  ...
                  exist_check=True,
                  use_prefix_id=False):
    return get_carla_info_path(idx, prefix, 'pose', '.txt', training,
                               relative_path, exist_check, use_prefix_id)
...
def get_timestamp_path(idx,
                       ...
                       exist_check=True,
                       use_prefix_id=False):
    return get_carla_info_path(idx, prefix, 'timestamp', '.txt', training,
                               relative_path, exist_check, use_prefix_id)
...
def get_carla_image_info(path,
    ...
    root_path = Path(path)
    if training:
        image_ids = _read_imageset_file(path + '/train/trainval.txt')
    else:
        image_ids = _read_imageset_file(path + '/val/trainval.txt')
...
def carla_anno_to_label_file(annos, folder):
    folder = Path(folder)
    for anno in annos:
        ...
        for j in range(anno['bbox'].shape[0]):
            ...
            label_line = carla_result_line(label_dict)
...
def carla_result_line(result_dict, precision=4):
